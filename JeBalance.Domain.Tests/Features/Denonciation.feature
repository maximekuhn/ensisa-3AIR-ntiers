Feature: Denonciation
    ...
    Scenario: Create a tax evasion denunciation
        Given a suspect
        And an informant
        And an offense type "TaxEvasion"
        And an evasion country "France"
        When the denunciation is created
        Then the denunciation has an id
        And the denunciation status is "WaitingForResponse"

    Scenario: Create a tax evasion denunciation should have correct parameters
        Given a suspect
        And an informant
        And an offense type "TaxEvasion"
        And an evasion country "France"
        When the denunciation is created
        Then the denunciation offense type is "TaxEvasion"
        And the evasion country is "France"

    Scenario: Can not create a tax evasion denunciation without a country
        Given a suspect
        And an informant
        And an offense type "Tax Evasion"
        When the denunciation is created
        Then the error "Tax evasion offense must have an evasion country" should be raised
    #
